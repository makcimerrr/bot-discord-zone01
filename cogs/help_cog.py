...
def check(reaction, user, message, bot):
    """Fonction pour vérifier la réaction ✅ sur un message"""
    return (
            user != bot.user
...
class SupremeHelpCommand(commands.Cog, commands.HelpCommand):
    """Commande d'aide personnalisée avec embeds et réactions"""

    def get_command_signature(self, command):
        return '%s%s %s' % (self.context.clean_prefix, command.qualified_name, command.signature)

    async def send_bot_help(self, mapping):
        """Affiche toutes les commandes disponibles par catégorie"""
    ...
    async def send_help_embed(self, title, description, commands_list):
        """Envoie un embed d'aide générique"""
        await self.context.message.delete()
        ...
        if filtered_commands := await self.filter_commands(commands_list):
    ...
    async def send_cog_help(self, cog):
        """Affiche l'aide pour un cog spécifique"""
